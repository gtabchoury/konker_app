...
class RestDestinationService {
  static Future<List<RestDestination>> getAll(
      String applicationName, String authToken) async {
    var headers = {'Authorization': 'Bearer $authToken'};

    http.Response response = await http.get(
        Uri.parse(
            '${Constants.BASE_API_URL}/$applicationName/restDestinations/'),
        headers: headers);

    if (response.statusCode == 200) {
      Iterable l = json.decode(response.body)['result'];
      return List<RestDestination>.from(
          l.map((model) => RestDestination.fromJson(model)));
    } else {
      throw Exception("Erro ao obter destinos Rest");
    }
  }

  static Future<RestDestination> getByGuid(
      String guid, String applicationName, String authToken) async {
    var headers = {'Authorization': 'Bearer $authToken'};

    http.Response response = await http.get(
        Uri.parse(
            '${Constants.BASE_API_URL}/$applicationName/restDestinations/$guid/'),
        headers: headers);
  ...
  static Future<RestDestination> create(RestDestination restDestination,
      String applicationName, String authToken) async {
    var headers = {
  ...
  static Future<bool> edit(RestDestination restDestination, String guid,
      String applicationName, String authToken) async {
    var headers = {
      'Authorization': 'Bearer $authToken',
      'Content-Type': 'application/json'
  ...
  static Future<bool> delete(String restDestinationGuid, String applicationName,
    ...
    http.Response response = await http.delete(
      Uri.parse(
          '${Constants.BASE_API_URL}/$applicationName/restDestinations/$restDestinationGuid'),
      headers: headers,
    );
